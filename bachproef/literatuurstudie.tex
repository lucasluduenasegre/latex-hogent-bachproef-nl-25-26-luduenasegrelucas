...
De labo-omgeving voor dit \emph{OLOD} is gebaseerd op de ``\emph{Ansible Skeleton}'' van \textcite{Vreckem2025}: een sjabloon voor \emph{Ansible}-projecten met een geïntegreerde \emph{Vagrant}-omgeving (zie \nameref{subsec:bertvv}). Deze structuur -- met mappen zoals ``\texttt{ansible/}'', ``\texttt{ansible/host\_vars/}'', ``\texttt{ansible/group\_vars/}'' en een centrale ``\texttt{site.yml}''-\emph{playbook} -- vormt ook de uitvoerstructuur van de \emph{converter} die in deze bachelorproef wordt ontwikkeld \autocite{HoGentTIN2025a}.
...
\begin{description}
  \item[``\texttt{bertvv.rh-base}''] Basisinstallatie en -configuratie van een \emph{RedHat}-gebaseerde server: firewall, \emph{SELinux}, gebruikersbeheer en algemene systeeminstellingen \autocite{Vreckem2016}.
...
De \emph{parser} neemt de \emph{tokens} van de vorige fase als invoer en controleert of deze voldoen aan de grammaticale regels van de brontaal \autocite{Seidl2012}. Het resultaat is een \emph{parse tree}, die de hiërarchische structuur van de broncode weergeeft (zie \nameref{subsec:parsetree}). Als de \emph{tokens} niet grammaticaal correct zijn, ontstaat er in de \emph{parser} een syntaxfout.

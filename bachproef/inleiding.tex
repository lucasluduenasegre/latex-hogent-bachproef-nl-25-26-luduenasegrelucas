...
In Hoofdstuk~\ref{ch:manuele_testomgeving} (\nameref{ch:manuele_testomgeving}) wordt het ontwerp en de implementatie van de manuele testomgeving voor de proof-of-concept beschreven. Dit bevat een evaluatie van bestaande tools in de betrokken vakgebieden (\emph{UML} en \emph{configuration management}), alsook het ontwerp van de verwachte invoer en uitvoer voor de \emph{converter}.

In Hoofdstuk~\ref{ch:poc} (\nameref{ch:poc}) wordt toegelicht hoe de proof-of-concept, in de vorm van een \emph{converter} voor \emph{PlantUML} naar \emph{Ansible} (\emph{PlantUML2Ansible}), is ontwikkeld.

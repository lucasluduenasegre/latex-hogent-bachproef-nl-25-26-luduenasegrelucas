...
Tenslotte biedt het OLOD ``Infrastructure Automation'' een reeks virtuele labo's die verschillende tools (Jenkins\footnote{\url{https://www.jenkins.io/}}, Ansible, Prometheus\footnote{\url{https://prometheus.io/}} en Kubernetes\footnote{\url{https://kubernetes.io/}}) introduceren voor het automatisch opzetten, configureren en monitoren van serveromgevingen. Nogmaals, hierbij is het gebruik van UML-diagrammen niet verplicht, maar aangeraden.
...
``Source-to-source compilers'', algemeen gekend als ``transpilers'', zijn compilers die code geschreven in één high-level (programmeer-)taal als invoer aannemen en deze als uitvoer omzetten naar code in een andere high-level taal. De use cases voor deze software kunnen zich ook uitbreiden tot het optimaliseren of omzetten van code naar een andere/nieuwere versie van eenzelfde taal. \autocite{Ilyushin2016}
...
Bij het begin van het onderzoek zal er parallel met de andere voorbereidende fases (1-3) een studie uitgevoerd worden over hoe transpilers werken en hoe deze ontwikkeld kunnen worden.
...
Hier zal er een afweging gemaakt worden voor de verschillende tools binnen UML-modeling en ``configuration management'' die gebruikt zullen worden tijdens de ontwikkeling van de transpiler.

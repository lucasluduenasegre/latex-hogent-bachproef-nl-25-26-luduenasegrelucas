...
Binnen de opleiding Toegepaste Informatica op HOGENT hebben de meeste OLODs (opleidingsonderdelen) gelinkt aan het keuzetraject ``System \& Network Administrator'' de opzet om te leren (virtuele) serveromgevingen te creeëren en beheren. Bij het opzetten van dit soort infrastructuur, is helderheid over elke stap van het proces en communicatie met elke -- al dan niet technisch ingestelde -- stakeholder cruciaal.
...
Hoe kan men een gegeven netwerk- en implementatiediagram (ontworpen met bv. PlantUML\footnote{\url{https://plantuml.com/}}) van een (virtuele) serveromgeving rechtstreeks omzetten (met minimale menselijke interventie) tot een overeenkomende, gebruiksklare ``configuration management''-ondersteunde (bv. met Ansible) serveromgeving?
...
``Configuration management'' heeft als principe om een (grote) set van manuele, repetitieve configuratietaken (installeren van packages, beheren van firewalls, instellen van de default gateways en DNS-servers, \ldots) automatisch te laten uitvoeren. In het beste geval verloopt dit proces op een idempotente manier; het herhalen van het hetzelde proces geeft exact hetzelfde resultaat. Deze manier van werken schaalt beter voor omgevingen met honderden of duizenden servers, aangezien het aanzienlijk minder tijdrovend en vatbaar voor menselijke fouten is. \autocite{Likitha2022}
...
``Source-to-source compilers'', algemeen gekend als ``transpilers'', zijn compilers die code geschreven in één high-level (programmeer-)taal als invoer aannemen en deze als uitvoer omzetten naar code in een andere high-level taal. De use cases voor deze software kunnen zich ook uitbreiden tot het omzetten van code van naar een andere/nieuwere versie van eenzelfde taal of het optimaliseren van code. \autocite{Ilyushin2016}
...
Deze bachelorproef zal uitgevoerd worden in de volgende fases, van 9 februari t.e.m. 22 mei, 2026:
...
\begin{itemize}
  ...
  \item{Fase 4 (week 8-15): Ontwikkeling van de proof-of-concept transpiler}
...
Indien een fase begint met hetzelfde nummer maar eindigt met een ander letter, indiceert dit dat ze op hetzelfde moment (kunnen) beginnen, eindigen (met fases 1A, 3C en 5 als uitzondering) en parallel worden uitgevoerd. Week 1 begint op maandag 9 februari, 2026.

\subsection{Fase 1A (week 1-7): Studie over (het ontwikkelen van) transpilers}%
\label{subsec:studie_ontwikkelen_transpilers}

Bij het begin van het onderzoek zal er parallel met de andere voorbereidende fases (1-3) een studie uitgevoerd worden over hoe deze werken en hoe deze ontwikkeld kunnen worden.
...
Hier zal er een afweging gemaakt worden (samen met de co-promotor) voor de verschillende tools binnen UML-modeling en ``configuration management'' die gebruikt zullen worden tijdens de ontwikkeling van de proof-of-concept transpiler.
...
Tijdens deze fase wordt de infrastructuur (ontworpen in fase 2) opgezet en geconfigureerd met zowel ``Infrastructure-as-Code''-tool(s) als ``configuration management''. Er wordt intussen ook gedocumenteerd hoe het ``configuration management''-gedeelte wordt opgezet, want dit vormt de target output voor de proof-of-concept transpiler. De uitvoer van de transpiler zal dan (uiteraard) worden toegepast op de omgeving minus het ``configuration management''-gedeelte.

\subsection{Fase 3B (week 6-7): UML-diagrammen herschrijven naar PlantUML/MermaidJS}%
\label{subsec:uml_diagrammen_herscrijven}

Nadat de netwerk- en implementatiediagrammen zijn ontworpen, zullen deze herschreven worden in PlantUML/MermaidJS. Deze diagrammen dienen een tweevoudig doel; ze kunnen binnen de originele context van hun taal als visuele UML-diagrammen worden gegenereerd, en ze vormen de bron-bestanden voor de proof-of-concept transpiler, die deze zal omzetten tot ``configuration management''-code (in Ansible/Puppet/Chef).

\subsection{Fase 3C (week 6-10): Opstellen van testcases}%
\label{subsec:opstellen_testcases}

Hier worden de testcases opgesteld waaraan de proof-of-concept transpiler aan moet voldoen. De testen zullen hoofdzakelijk buigen over de kwaliteit van de uitvoer (aantal playbooks, idempotentie, grootte van codebase, \ldots) en de werking van de finale infrastructuur. Deze fase zal in de praktijk waarschijnlijk gedeeltelijk parallel verlopen met fase 4, aangezien er tijdens het ontwikkelen van de oplossing mogelijk nieuwe testcases kunnen ontstaan.

\subsection{Fase 4 (week 8-15): Ontwikkeling van de proof-of-concept transpiler}%
\label{subsec:ontwikkeling_poc_transpiler}

Tijdens deze fase wordt de proof-of-concept transpiler ontwikkeld. Deze zal als invoer de netwerk- en implementatiediagrammen (ontworpen met PlantUML/MermaidJS) aannemen en deze omzetten tot ``configuration management''-code (in Ansible/Puppet/Chef). Deze fase loopt tot 22 mei, 2026, aangezien dit ook de laatste dag van de stage voor de auteur is.

\subsection{Fase 5 (week 11-15): Testen van de bekomen oplossing}%
\label{subsec:evaluatie_bekomen_oplossing}

Tenslotte worden de opgestelde testen uit fase 3C uitgevoerd op de (gedeeltelijk) uitgewerkte proof-of-concept transpiler. Op basis van de resultaten deze testen zal er een rapport opgesteld worden die aangeeft in welke mate deze oplossing een antwoord biedt op de hoofdonderzoeksvraag. Nogmaals, dit zal allicht parallel verlopen met fase 4.
...
\subsection{Verwachte functionaliteit voor de proof-of-concept transpiler}%
\label{subsec:verwachte_functionaliteit_poc}

% Aangezien de auteur weinig ervaring heeft met het softwareontwikkeling en nog minder met het ontwikkelen van transpilers, is het mogelijk dat er tijdens fase 1A alternatieven gezocht zullen worden voor het ontwikkelen van een oplossing. Dit onderzoek situeert zich nu eenmaal hoofdzakelijk in de specialisatierichting ``System \& Network Administrator'' en niet in ``Mobile \& Enterprise development'', dus de focus zal sowieso eerder liggen bij de kwaliteit van de bekomen infrastructuur dan bij de cohesie van de geprogrammeerde transpiler.

De verwachting van deze oplossing is om met de bekomende ``configuration management''-code een minimaal werkende infrastructuur te verkrijgen die de beschrijving van de gegeven netwerk- en implementatiediagrammen zo nauw mogelijk benadert.
Aangezien de syntax van zowel PlantUML als Ansible heel uitgebreid is, zal de transpiler zich slechts over een (relevant) deel hiervan buigen. Er zullen enkel netwerk- en implementatiediagrammen als invoer kunnen gebruikt worden, en voor de Ansible playbooks als uitvoer zal er een deelverzameling van ondersteunde ``collections'' en ``roles'' worden gespecifiëerd. Nogmaals, dit wordt een proof-of-concept, dus het onderzoek zal ook aantonen waar het potentiëel tot uitbreidingen ligt, alsook de limitaties van welke informatie uit een UML-diagram rechtstreeks kan omgezet worden naar ``configuration management'' en vice versa.
